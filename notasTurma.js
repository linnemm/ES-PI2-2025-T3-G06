document.addEventListener("DOMContentLoaded", () => {
  ...
  function adicionarNota(matricula, nome, componente, nota) {
    ...
    novaLinha.innerHTML = `
      <td><input type="checkbox" class="chkNota"></td>
      <td>${matricula}</td>
      <td>${nome}</td>
  ...
  document.getElementById("selecionarTodos").addEventListener("change", (e) => {
    ...
    tabela.querySelectorAll(".chkNota").forEach(chk => chk.checked = marcado);
  });
  ...
  function abrirMenu(tipo){
    selectContainer.innerHTML=""; btnIr.style.display="none"; menuFlutuante.style.display="block";
    if(tipo==="turma"){
      ...
      document.getElementById("selInstituicao").addEventListener("change",()=>{
        ...
        document.getElementById("selCurso").addEventListener("change",()=>{
          ...
          document.getElementById("selDisciplina").addEventListener("change",()=>{
            ...
            btnIr.onclick=()=>window.location.href="notasTurma.html";
          });
        });
      });
    }
